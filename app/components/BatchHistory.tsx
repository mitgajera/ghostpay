            ...
            <div>
              <div className="flex items-center gap-2.5">
                <span className="font-display font-semibold text-sm text-gp-ghost">
                  {(b.totalUsdc / 1_000_000).toFixed(2)} USDC
                </span>
              ...
              <table className="w-full text-xs font-mono">
                <thead>
                  <tr className="border-b border-gp-border">
                    {["Name", "Wallet", "Currency", "USDC"].map((h, i) => (
                      <th key={h} className={`pb-2 font-normal text-[9px] text-gp-ghost-dim/50 uppercase tracking-widest ${i >= 3 ? "text-right" : "text-left"}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {b.recipients.map((r, i) => (
                    <tr key={i} className="border-b border-gp-border/40">
                      ...
                      <td className="py-2 text-gp-ghost-dim">{r.currency}</td>
